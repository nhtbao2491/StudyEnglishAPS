...
const STATUS_COLORS = { Completed: 'badge-completed', 'In Progress': 'badge-inprogress', Started: 'badge-started' };
    ...
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.sub}>Total: <b>{vocabList.length}</b> words</p>
        </div>
        <div className={styles.headerBtns}>
          ...
          <button className={styles.btnPrimary} onClick={() => { setEditWord(null); setShowAddWord(true); }}>
      ...
      <div className={styles.statsBar}>
        <div className={styles.statPill} style={{ background: '#fde8e8', color: '#c0392b' }}>
          High: <b>{vocabList.filter(v => v.priority === 'High').length}</b>
        </div>
        <div className={styles.statPill} style={{ background: '#fef3c7', color: '#b7791f' }}>
          Medium: <b>{vocabList.filter(v => v.priority === 'Medium').length}</b>
        </div>
        <div className={styles.statPill} style={{ background: '#d4edda', color: '#2d6a4f' }}>
          Low: <b>{vocabList.filter(v => v.priority === 'Low').length}</b>
        </div>
        <div className={styles.statPill} style={{ background: '#d1fae5', color: '#065f46' }}>
          Completed: <b>{vocabList.filter(v => v.status === 'Completed').length}</b>
        </div>
        <div className={styles.statPill} style={{ background: '#fef3c7', color: '#92400e' }}>
          In Progress: <b>{vocabList.filter(v => v.status === 'In Progress').length}</b>
        </div>
      </div>
      ...
      >
        <table className={styles.table}>
          ...
          <tbody>
            {filtered.map((v, idx) => (
              <tr key={v.id} className={styles.row}>
                <td className={styles.num}>
                  ...
                  ><Icon name="speak" size={15} color="var(--green-dark)" /></button>
                </td>
                <td className={styles.wordCell}><b>{v.word}</b></td>
                ...
                <td>
                  <span className={`tag ${PRIORITY_COLORS[v.priority]}`}>{v.priority}</span>
                </td>
                <td>{v.topic}</td>
                <td>
                  <span className={v.done ? styles.yes : styles.no}>{v.done ? 'Yes' : 'No'}</span>
                </td>
                <td className={styles.score}>{v.score}%</td>
                <td>
                  <span className={styles.masteryBadge} data-level={v.mastery}>{v.mastery}</span>
                </td>
                <td>{v.dateAdded}</td>
                <td>{v.dateCompleted || '-'}</td>
                <td>{v.duration} days</td>
                <td>
                  <span className={`tag ${STATUS_COLORS[v.status]}`} style={{whiteSpace:'nowrap'}}>{v.status}</span>
                </td>
                <td>
                  <div className={styles.actions}>
                  <button className={styles.editBtn} onClick={() => { setEditWord(v); setShowAddWord(true); }}><Icon name="edit" size={14} color="#4a7c59" /></button>
                  <button className={styles.delBtn} onClick={() => deleteWord(v.id)}><Icon name="trash" size={14} color="#c0392b" /></button>
                  </div>
                </td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr>
                <td colSpan={19} className={styles.empty}>No words found. Add your first word! 🌱</td>
              </tr>
            )}
          </tbody>
        </table>
